...
def add_rse(rse, issuer, deterministic=True, volatile=False, city=None, region_code=None,
            ...
            availability=None, supported_chksums=None):
    """
    ...
    return rse_module.add_rse(rse, deterministic=deterministic, volatile=volatile, city=city,
                              ...
                              longitude=longitude, ASN=ASN, availability=availability, supported_chksums=supported_chksums)
...
def del_rse_checksum(rse, chksum_name, issuer):
    """
    Delete a RSE attribute.
    ...
    return del_rse_attribute(rse=rse, key=get_chksum_attr_key(chksum_name), issuer=issuer)
...
def add_rse_checksum(rse, chksum_name, issuer):
    """ Adds a RSE attribute.
    ...
    returns: True if successful, False otherwise.
    """
    if is_chksum_valid(chksum_name):
        return add_rse_attribute(rse=rse, key=get_chksum_attr_key(chksum_name), value=True, issuer=issuer)
    else:
        return False
