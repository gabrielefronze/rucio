...
from rucio.common.utils import build_url, is_chksum_valid, get_chksum_attr_key
...
class RSEClient(BaseClient):
    ...
    def add_rse_checksum(self, rse, chksum_name):
        """
        ...
        if is_chksum_valid(chksum_name):
            return self.add_rse_attribute(rse=rse, key=get_chksum_attr_key(chksum_name), value=True)
        else:
            return False
    ...
    def delete_rse_checksum(self, rse, chksum_name):
        """
        Sends the request to delete a RSE attribute.
        ...
        :return: True if RSE attribute was deleted successfully else False.
        """
        return self.delete_rse_attribute(rse=rse, key=get_chksum_attr_key(chksum_name))
